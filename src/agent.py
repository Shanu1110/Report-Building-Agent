...
def classify_intent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    prompt = get_intent_classification_prompt().format(
        ...
        conversation_history=state.get("conversation_history", history),
    )
...
def should_continue(state: AgentState) -> str:
    """Return the next graph node selected by intent classification."""
    return state.get("next_step", "end")

def create_workflow(llm, tools):
    """Create and compile the document assistant workflow."""
