...
class SimulatedRetriever:
    ...
    def reset_to_sample_documents(self) -> None:
        """Restore the built-in sample document collection."""
        self.documents = {}
        self._load_sample_documents()

    def load_pdf(self, file_path: str) -> Document:
        """Replace the active collection with text extracted from one PDF."""
        ...
        document = Document(
            doc_id=f"PDF-{uuid.uuid4().hex[:8].upper()}",
            title=path.name,
            content=content,
            doc_type="pdf",
            metadata={
