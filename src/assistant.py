...
class DocumentAssistant:
    ...
    def start_session(self, user_id: str, session_id: Optional[str] = None) -> str:
        ...
        self.tool_logger.set_session_id(session_id)
        return session_id

    def upload_pdf(self, file_path: str) -> Dict[str, Any]:
        """Load one PDF and start a fresh isolated session for it."""
        document = self.retriever.load_pdf(file_path)
        user_id = self.current_session.user_id if self.current_session else "demo_user"
        session_id = self.start_session(user_id)
        self.current_session.document_context = [document.doc_id]
        self._save_session()
        return {
            "session_id": session_id,
    ...
    def reset_documents(self) -> str:
        """Restore sample documents and start a fresh isolated session."""
        self.retriever.reset_to_sample_documents()
        user_id = self.current_session.user_id if self.current_session else "demo_user"
    ...
    def _get_conversation_summary(self, config) -> str:
        ...
        summary = current_state.get("conversation_summary", "No previous conversation.")
        return summary
    ...
    # The checkpointer uses the session ID to retain state between turns.
        config = {
            "configurable": {
        ...
        try:
            ...
            if final_state.get("messages"):
                ...
                self.current_session.conversation_history.append({
                    ...
                    "intent": final_state.get("intent").model_dump(mode="json")
