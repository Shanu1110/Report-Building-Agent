...
def print_help():
    ...
    print("  /upload   - Load one PDF and use it as the only document source")
    print("  /reset    - Restore the built-in sample documents")
    ...
    while True:
        try:
            ...
            if user_input.lower() == "/quit":
                ...
                session_id = assistant.reset_documents()
                print(f"\nSample documents restored. New session: {session_id}", color='green')
                continue
            elif user_input.lower().startswith("/upload"):
                pdf_path = user_input[len("/upload"):].strip()
                if len(pdf_path) >= 2 and pdf_path[0] == pdf_path[-1] and pdf_path[0] in "\"'":
                    pdf_path = pdf_path[1:-1]
                if not pdf_path:
                    print("\nUsage: /upload path\\to\\document.pdf", color='yellow')
                    continue
                try:
                    upload = assistant.upload_pdf(pdf_path)
                    print(
                        f"\nLoaded {upload['title']} ({upload['page_count']} pages). "
                        f"Only this PDF is now available. New session: {upload['session_id']}",
                        color='green',
                    )
                except Exception as exc:
                    print(f"\nUpload failed: {exc}", color='red')
                continue
